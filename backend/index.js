...
// ─── PRINTING MODULE ENDPOINTS ──────────────────────────────────────────────

let printRequests = [
  {
    id: 'print-101',
    studentName: 'Arjun K.',
    fileName: 'Lab_Report_S4.pdf',
    filename: 'Lab_Report_S4.pdf',
    copies: 2,
    printType: 'B&W',
    layout: 'Single Side',
...
app.post('/api/print/submit', upload.single('file'), (req, res) => {
  try {
    if (!req.file) {
    ...
    const pType = printType || 'B&W';
    const pLayout = layout || 'Single Side';
    const sName = studentName || 'Student User';

    const ratePerCopy = pType === 'Color' ? 5 : 2;
    const price = numCopies * ratePerCopy;
    ...
    console.error('Print submit error:', error);
...
app.put('/api/print/:id/status', (req, res) => {
  const { id } = req.params;
  const { status } = req.body;

  let target = printRequests.find(r => String(r.id) === String(id));
  if (target) {
